...
del missing_ratio, full_features
gc.collect()
...
del x_train_encoded, x_test_encoded, feature_names, cat_indices, categorical_indices
gc.collect()

print(f"Dimensions réduites - Train: {x_train.shape}, Test: {x_test.shape}")
...
del corr_matrix, to_remove
gc.collect()

# CORRECTION : Supprimer cette ligne qui n'est pas utilisée
...
sig_treshold = 0.8
gammas = np.arange(0, 1, 0.25)
lambdas = np.arange(0, 1, 0.25)
best_score = 0
n_iter = 100
lambda_ = 0.1
gamma = 0.2
